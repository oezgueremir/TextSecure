...
public class DateUtils extends android.text.format.DateUtils {
  ...
  private static boolean isWithin24H(final long millis) {
    return System.currentTimeMillis() - millis <= DAY_IN_MILLIS;
  }
  ...
  public static String getBetterRelativeTimeSpanString(final Context c, final long millis) {
    int formatFlags;
    if (isToday(millis)) {
      formatFlags = DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_ABBREV_TIME;
    } else if (isWithin24H(millis)) {
      formatFlags = DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_ABBREV_TIME | DateUtils.FORMAT_SHOW_WEEKDAY | DateUtils.FORMAT_ABBREV_WEEKDAY;
